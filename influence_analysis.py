...
def calculate_influence(df, infer, model, sample_size=2000):
    ...
    variables_to_test = [v for v in ['Smoking','Diabetes','Alcohol','BMI','Hypertension'] if v in model_nodes]
    available_vars = [v for v in ['Age','Sex','Smoking','Alcohol','Diabetes','Hypertension','BMI'] if v in model_nodes]
    ...
    for var in variables_to_test:
        ...
        for _, r in sample.iterrows():
            evidence = {k: v for k, v in r[available_vars].to_dict().items() if k != var}
            ...
            if var in ['Smoking','Diabetes','Hypertension']:
                ...
                toggled = 'Obese' if evidence.get(var, 'Normal')!='Obese' else 'Normal'
...
def plot_influence_ranking(deltas):
    influence = pd.DataFrame({'variable': list(deltas.keys()), 'mean_delta': list(deltas.values())})
